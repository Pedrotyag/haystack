...
RSpec.describe Haystack::Faraday do
  before(:all) do
  ...
  after(:all) do
    Haystack.configuration.enabled_patches = Haystack::Configuration::DEFAULT_PATCHES
  end
    ...
    context "with custom trace_propagation_targets" do
      ...
      context "when the request is not to the same target" do
        ...
        it "doesn't add haystack headers to outgoing requests to different target" do
          ...
          expect(response.headers.key?("haystack-trace")).to eq(false)
      ...
      context "when the request is to the same target" do
        ...
        before do
          Haystack.configuration.trace_propagation_targets = ["example.com"]
        end

        it "adds haystack headers to outgoing requests" do
          transaction = Haystack.start_transaction
          Haystack.get_current_scope.set_span(transaction)
          ...
          expect(response.env.request_headers.key?("haystack-trace")).to eq(true)
      ...
      context "when the request's url configured target regexp" do
        ...
        before do
          Haystack.configuration.trace_propagation_targets = [/example/]
        end
  ...
  context "when Haystack is not initialized" do
    let(:http) do
      Faraday.new(url) do |f|
        f.adapter Faraday::Adapter::Test do |stub|
          stub.get("/test") do

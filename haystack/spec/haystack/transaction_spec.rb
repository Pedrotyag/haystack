...
RSpec.describe Haystack::Transaction do
  before do
    perform_basic_setup
  end
  ...
  describe ".from_haystack_trace" do
    let(:haystack_trace) { subject.to_haystack_trace }
    ...
    context "when tracing is enabled (> 0)" do
      ...
      it "returns correctly-formatted value" do
        child_transaction = described_class.from_haystack_trace(haystack_trace, op: "child")
      ...
      it "handles invalid values without crashing" do
        child_transaction = described_class.from_haystack_trace("dummy", op: "child")

        expect(child_transaction).to be_nil
      end

      it "stores frozen empty baggage on incoming traces from older SDKs" do
        child_transaction = described_class.from_haystack_trace(haystack_trace, baggage: nil, op: "child")
      ...
      it "stores correct baggage on incoming baggage header" do
        child_transaction = described_class.from_haystack_trace(haystack_trace, baggage: baggage, op: "child")
        expect(child_transaction.baggage).not_to be_nil
        expect(child_transaction.baggage.mutable).to be(false)
    ...
    context "when tracing is disabled" do
      ...
      it "returns nil" do
        expect(described_class.from_haystack_trace(haystack_trace, op: "child")).to be_nil
      end
    end
  end
  ...
  describe "#set_initial_sample_decision" do
    ...
    context "when tracing is not enabled" do
      before do
        allow(Haystack.configuration).to receive(:tracing_enabled?).and_return(false)
      end

      it "sets @sampled to false and return" do
        allow(Haystack.configuration).to receive(:tracing_enabled?).and_return(false)

        transaction = described_class.new(sampled: true, hub: Haystack.get_current_hub)
    ...
    context "when tracing is enabled" do
      let(:subject) { described_class.new(op: "rack.request", hub: Haystack.get_current_hub) }

      before do
        allow(Haystack.configuration).to receive(:tracing_enabled?).and_return(true)
      end
      ...
      context "when the transaction already has a decision" do
        it "doesn't change it" do
          ...
          transaction = described_class.new(sampled: false, hub: Haystack.get_current_hub)
          transaction.set_initial_sample_decision(sampling_context: {})
          expect(transaction.sampled).to eq(false)
      ...
      context "when traces_sampler is not set" do
        before do
          Haystack.configuration.traces_sample_rate = 0.5
        end
        ...
        it "accepts integer traces_sample_rate" do
          Haystack.configuration.traces_sample_rate = 1
      ...
      context "when traces_sampler is provided" do
        it "prioritizes traces_sampler over traces_sample_rate" do
          ...
          Haystack.configuration.traces_sampler = ->(_) { false }
        ...
        it "ignores the sampler if it's not callable" do
          Haystack.configuration.traces_sampler = ""
        ...
        it "discards the transaction if generated sample rate is not valid" do
          Haystack.configuration.traces_sampler = ->(_) { "foo" }
          subject.set_initial_sample_decision(sampling_context: {})
        ...
        it "uses the genereted rate for sampling (positive)" do
          expect(Haystack.configuration.logger).to receive(:debug).exactly(3).and_call_original

          Haystack.configuration.traces_sampler = ->(_) { true }
          subject = described_class.new(hub: Haystack.get_current_hub)
          subject.set_initial_sample_decision(sampling_context: {})
          expect(subject.sampled).to eq(true)
          expect(subject.effective_sample_rate).to eq(1.0)

          Haystack.configuration.traces_sampler = ->(_) { 1.0 }
          subject = described_class.new(hub: Haystack.get_current_hub)
          subject.set_initial_sample_decision(sampling_context: {})
          expect(subject.sampled).to eq(true)
          expect(subject.effective_sample_rate).to eq(1.0)

          Haystack.configuration.traces_sampler = ->(_) { 1 }
          subject = described_class.new(hub: Haystack.get_current_hub)
          subject.set_initial_sample_decision(sampling_context: {})
          expect(subject.sampled).to eq(true)
          expect(subject.effective_sample_rate).to eq(1.0)
        ...
        it "uses the genereted rate for sampling (negative)" do
          expect(Haystack.configuration.logger).to receive(:debug).exactly(2).and_call_original
          ...
          Haystack.configuration.traces_sampler = ->(_) { 0.0 }
          subject = described_class.new(hub: Haystack.get_current_hub)
          subject.set_initial_sample_decision(sampling_context: {})
          expect(subject.sampled).to eq(false)
          expect(subject.effective_sample_rate).to eq(0.0)
      ...
      context "when backpressure handling is enabled" do
        ...
        it "uses downsampled rate to sample" do
          expect(Haystack.get_current_client.transport).to receive(:any_rate_limited?).and_return(true)
          Haystack.backpressure_monitor.run
  ...
  describe "#finish" do
    let(:events) do
      Haystack.get_current_client.transport.events
    end

    let(:another_hub) do
      Haystack.get_current_hub.clone
    end
    ...
    it "finishes the transaction with explicit timestamp" do
      timestamp = Haystack.utc_now.to_f
    ...
    it "only follows sampling decision" do
      Haystack.configuration.traces_sampler = proc { false }
      subject = described_class.new(parent_sampled: true, hub: Haystack.get_current_hub)
    ...
    context "if the transaction is not sampled" do
      subject { described_class.new(sampled: false, hub: Haystack.get_current_hub) }
      ...
      it "records lost event with reason sample_rate" do
        subject.finish
        expect(Haystack.get_current_client.transport).to have_recorded_lost_event(:sample_rate, 'transaction')
        expect(Haystack.get_current_client.transport).to have_recorded_lost_event(:sample_rate, 'span')
      end
    end
    ...
    context "when backpressure handling is enabled and transaction is not sampled" do
      ...
      it "records lost event with reason backpressure" do
        ...
        subject.finish
        expect(Haystack.get_current_client.transport).to have_recorded_lost_event(:backpressure, 'transaction')
        expect(Haystack.get_current_client.transport).to have_recorded_lost_event(:backpressure, 'span')
      end
    end

    context "if the transaction doesn't have a name" do
      subject { described_class.new(sampled: true, hub: Haystack.get_current_hub) }
  ...
  describe "#get_baggage" do
    ...
    context "when incoming empty frozen baggage from old SDK" do
      let(:incoming_baggage) { Haystack::Baggage.new({}, mutable: false) }
    ...
    context "when incoming baggage with haystack items" do
      let(:incoming_baggage) do
        Haystack::Baggage.from_incoming_header("haystack-trace_id=12345,foo=bar")
      end
    ...
    context "when incoming baggage with no haystack items" do
      let(:incoming_baggage) do
        Haystack::Baggage.from_incoming_header("foo=bar")
      end

      it "populates haystack baggage" do
        expect(subject).to receive(:populate_head_baggage).and_call_original

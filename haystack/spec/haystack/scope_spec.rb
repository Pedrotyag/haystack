...
RSpec.describe Haystack::Scope do
  ...
  let(:hub) do
    Haystack::Hub.new(client, subject)
  end

  describe "#initialize" do
    it "contains correct defaults" do
      expect(subject.breadcrumbs).to be_a(Haystack::BreadcrumbBuffer)
      ...
      expect(subject.propagation_context).to be_a(Haystack::PropagationContext)
    end
  ...
  describe "#dup" do
    ...
    it "deep-copies span as well" do
      ...
      span = Haystack::Transaction.new(sampled: true, hub: hub)
  ...
  describe "#clear" do
    it "resets the scope's data" do
      ...
      subject.set_span(Haystack::Transaction.new(op: "foo", hub: hub))
  ...
  describe "#get_transaction & #get_span" do
    ...
    let(:transaction) do
      Haystack::Transaction.new(op: "parent", hub: hub)
    end
  ...
  describe "#apply_to_event" do
    ...
    it "applies the contextual data to event" do
      ...
      expect(event.breadcrumbs).to be_a(Haystack::BreadcrumbBuffer)
    ...
    it "sets trace context and dynamic_sampling_context from span if there's a span" do
      transaction = Haystack::Transaction.new(op: "foo", hub: hub)
  ...
  describe "#generate_propagation_context" do
    it "initializes new propagation context without env" do
      expect(Haystack::PropagationContext).to receive(:new).with(subject, nil)
      subject.generate_propagation_context
    end

    it "initializes new propagation context without env" do
      env = { foo: 42 }
      expect(Haystack::PropagationContext).to receive(:new).with(subject, env)

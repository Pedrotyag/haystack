...
RSpec.describe :redis_logger do
  ...
  context "config.send_default_pii = false" do
    before { Haystack.configuration.send_default_pii = false }

    it "adds Redis command breadcrumb with command and key" do
      result = redis.set("key", "value")

      expect(result).to eq("OK")
      expect(Haystack.get_current_scope.breadcrumbs.peek).to have_attributes(
        category: "db.redis",
  ...
  context "config.send_default_pii = true" do
    before { Haystack.configuration.send_default_pii = true }
  ...
  context "when DSN is nil" do
    before do
      Haystack.configuration.instance_variable_set(:@dsn, nil)
    end

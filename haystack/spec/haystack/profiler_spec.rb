...
RSpec.describe Haystack::Profiler, when: :stack_prof_installed? do
  before do
  ...
  let(:subject) { described_class.new(Haystack.configuration) }
  ...
  describe '#set_initial_sample_decision' do
    context 'with profiling disabled' do
      it 'does not sample when profiles_sample_rate is nil' do
        Haystack.configuration.profiles_sample_rate = nil
      ...
      it 'does not sample when profiles_sample_rate is invalid' do
        Haystack.configuration.profiles_sample_rate = 5.0
    ...
    context 'with profiling enabled' do
      ...
      it 'does not sample when profiles_sample_rate is 0' do
        Haystack.configuration.profiles_sample_rate = 0
      ...
      it 'uses profiles_sample_rate for sampling (positive result)' do
        Haystack.configuration.profiles_sample_rate = 0.5
  ...
  describe '#to_hash' do
    ...
    context 'with profiled code' do
      ...
      it 'has correct frames' do
        ...
        expect(foo_frame[:filename]).to eq('spec/haystack/profiler_spec.rb')
        expect(foo_frame[:abs_path]).to include('haystack/haystack/spec/haystack/profiler_spec.rb')
        ...
        expect(bar_frame[:filename]).to eq('spec/haystack/profiler_spec.rb')
        expect(bar_frame[:abs_path]).to include('haystack/haystack/spec/haystack/profiler_spec.rb')

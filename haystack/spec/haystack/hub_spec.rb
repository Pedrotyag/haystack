...
RSpec.describe Haystack::Hub do
  let(:string_io) { StringIO.new }
  let(:logger) do
    ::Logger.new(string_io)
  end
  let(:configuration) do
    config = Haystack::Configuration.new
  ...
  describe '#capture_message' do
    ...
    it "returns an Event instance" do
      expect(subject.capture_message(message)).to be_a(Haystack::ErrorEvent)
    end
  ...
  describe '#capture_exception' do
    ...
    it "returns an Event instance" do
      expect(subject.capture_exception(exception)).to be_a(Haystack::ErrorEvent)
    end
    ...
    # see https://github.com/gethaystack/haystack/issues/1323
  ...
  describe '#capture_event' do
    ...
    it "returns an Event instance" do
      expect(subject.capture_event(event)).to be_a(Haystack::ErrorEvent)
    end
    ...
    it "raises error when passing a non-exception object" do
      ...
      end.to raise_error(ArgumentError, 'expect the argument to be a Haystack::Event, got String ("String")')
    end
    ...
    context "when event is a transaction" do
      it "transaction.set_context merges and takes precedence over scope.set_context" do
        ...
        transaction = Haystack::Transaction.new(hub: subject, name: 'test')
  ...
  describe "#add_breadcrumb" do
    let(:new_breadcrumb) do
      new_breadcrumb = Haystack::Breadcrumb.new
  ...
  describe "#clone" do
    it "creates a new hub with the current hub's top layer" do
      ...
      expect(new_hub).not_to eq(subject)
      expect(new_hub.current_client).to eq(subject.current_client)
      expect(new_hub.current_scope).to be_a(Haystack::Scope)
      expect(new_hub.current_scope).not_to eq(subject.current_scope)
    end
  end

  describe "#bind_client & #unbind_client" do
    let(:new_client) { Haystack::Client.new(configuration) }
  ...
  describe "#push_scope" do
    it "pushes a new scope to the scope stack" do
      expect(subject.current_scope).to eq(scope)
      subject.push_scope
      expect(subject.current_scope).to be_a(Haystack::Scope)
      expect(subject.current_scope).not_to eq(scope)
    end
  ...
  describe '#last_event_id' do
    ...
    it 'only records last_event_id for error events' do
      ...
      transaction = Haystack::Transaction.new(name: "test transaction", op: "rack.request", hub: subject)

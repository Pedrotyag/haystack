...
RSpec.describe Haystack::Backtrace do
  ...
  it "calls backtrace_cleanup_callback if it's present in the configuration" do
    ...
    Haystack::Backtrace.parse(Thread.current.backtrace, configuration.project_root, configuration.app_dirs_pattern, &callback)

    expect(called).to eq(true)
  end

  it "#lines" do
    expect(parsed_backtrace.lines.first).to be_a(Haystack::Backtrace::Line)
  end
  ...
  it "==" do
    backtrace2 = Haystack::Backtrace.new(parsed_backtrace.lines)

...
RSpec.describe Haystack::Transport do
  let(:io) { StringIO.new }
  let(:logger) { Logger.new(io) }
  let(:configuration) do
    Haystack::Configuration.new.tap do |config|
      config.server = 'http://12345:67890@haystack.localdomain/haystack/42'
  ...
  describe "#serialize_envelope" do
    context "normal event" do
      ...
      it "generates correct envelope content" do
        ...
        expect(envelope_header_parsed).to eq({
          ...
          "dsn" => Haystack::TestHelper::DUMMY_DSN,
    ...
    context "transaction event" do
      let(:transaction) do
        Haystack::Transaction.new(name: "test transaction", op: "rack.request", hub: hub)
      end
    ...
    context "metrics/statsd item" do
      ...
      let(:envelope) do
        envelope = Haystack::Envelope.new
        envelope.add_item(
    ...
    context "oversized event" do
      context "due to breadcrumb" do
        ...
        before do
          event.breadcrumbs = Haystack::BreadcrumbBuffer.new(1000)
          1000.times do |i|
            event.breadcrumbs.record Haystack::Breadcrumb.new(category: i.to_s, message: "x" * Haystack::Event::MAX_MESSAGE_SIZE_IN_BYTES)
          end
          serialized_result = JSON.generate(event.to_hash)
          expect(serialized_result.bytesize).to be > Haystack::Envelope::Item::MAX_SERIALIZED_PAYLOAD_SIZE
        end

        it "removes breadcrumbs and carry on" do
          data, _ = subject.serialize_envelope(envelope)
          expect(data.bytesize).to be < Haystack::Envelope::Item::MAX_SERIALIZED_PAYLOAD_SIZE
        ...
        context "if it's still oversized" do
          before do
            1000.times do |i|
              event.contexts["context_#{i}"] = "s" * Haystack::Event::MAX_MESSAGE_SIZE_IN_BYTES
            end
          end
      ...
      context "due to stacktrace frames" do
        ...
        let(:in_app_pattern) do
          ...
          Regexp.new("^(#{project_root}/)?#{Haystack::Configuration::APP_DIRS_PATTERN}")
        end
        ...
        before do
          ...
          new_stacktrace = Haystack::StacktraceInterface.new(
            frames: frame_list_size.times.map do |zero_based_index|
              Haystack::StacktraceInterface::Frame.new(
                "/fake/path",
                Haystack::Backtrace::Line.parse("app.rb:#{zero_based_index + 1}:in `/'", in_app_pattern)
              )
            end,
          )
  ...
  describe "#send_envelope" do
    ...
    context "event with attachments" do
      ...
      before do
        event.attachments << Haystack::Attachment.new(filename: "test-1.txt", bytes: "test")
        event.attachments << Haystack::Attachment.new(path: fixture_path("attachment.txt"))
      end

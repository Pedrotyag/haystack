...
RSpec.describe Haystack::Metrics do
  before do
  ...
  let(:aggregator) { Haystack.metrics_aggregator }
  ...
  describe '.timing' do
    ...
    context 'with running transaction' do
      let(:transaction) { transaction = Haystack.start_transaction(name: 'metrics') }
      ...
      it 'starts a span' do
        expect(Haystack).to receive(:with_child_span).with(op: Haystack::Metrics::OP_NAME, description: 'foo', origin: Haystack::Metrics::SPAN_ORIGIN).and_call_original

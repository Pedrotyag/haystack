...
RSpec.describe Haystack::StacktraceInterface::Frame do
  describe "#initialize" do
    ...
    let(:lines) do
      Haystack::Backtrace.parse(raw_lines, configuration.project_root, configuration.app_dirs_pattern).lines
    end

    it "initializes a Frame with the correct info from the given Backtrace::Line object" do
      first_frame = Haystack::StacktraceInterface::Frame.new(configuration.project_root, lines.first)
      ...
      second_frame = Haystack::StacktraceInterface::Frame.new(configuration.project_root, lines.last)
    ...
    it "does not strip load path when strip_backtrace_load_path is false" do
      first_frame = Haystack::StacktraceInterface::Frame.new(configuration.project_root, lines.first, false)
      expect(first_frame.filename).to eq(first_frame.abs_path)
      expect(first_frame.filename).to eq(raw_lines.first.split(':').first)

      second_frame = Haystack::StacktraceInterface::Frame.new(configuration.project_root, lines.last, false)

...
RSpec.describe Haystack::Client do
  ...
  subject { Haystack::Client.new(configuration) }

  let(:transaction) do
    hub = Haystack::Hub.new(subject, Haystack::Scope.new)
  ...
  describe "#transport" do
    let(:configuration) { Haystack::Configuration.new }

    context "when transport.transport_class is provided" do
      before do
        configuration.dsn = Haystack::TestHelper::DUMMY_DSN
        configuration.transport.transport_class = Haystack::DummyTransport
      end

      it "uses that class regardless if dsn is set" do
        expect(subject.transport).to be_a(Haystack::DummyTransport)
      end
    end

    context "when transport.transport_class is not provided" do
      context "when dsn is not set" do
        subject { described_class.new(Haystack::Configuration.new) }
      ...
      context "when dsn is set" do
        ...
        it "returns HTTP transport object" do
          expect(subject.transport).to be_a(Haystack::HTTPTransport)
        end
      end
    end
  end
  ...
  describe "#spotlight_transport" do
    ...
    it "has a transport when true" do
      ...
      expect(described_class.new(configuration).spotlight_transport).to be_a(Haystack::SpotlightTransport)
    end
  end
  ...
  describe '#send_envelope' do
    let(:envelope) do
      envelope = Haystack::Envelope.new({ env_header: 1 })
    ...
    context 'when transport failure' do
      ...
      before do
        ...
        allow(subject.transport).to receive(:send_envelope).and_raise(Haystack::ExternalError.new("networking error"))
      end
  ...
  describe '#capture_envelope' do
    ...
    it 'queues envelope to background worker' do
      expect(Haystack.background_worker).to receive(:perform).and_call_original
  ...
  describe "#event_from_transaction" do
    let(:hub) do
      Haystack::Hub.new(subject, Haystack::Scope.new)
    end
    ...
    it "correct dynamic_sampling_context when incoming baggage header" do
      baggage = Haystack::Baggage.from_incoming_header(
      ...
      transaction = Haystack::Transaction.new(name: "test transaction", hub: hub, baggage: baggage, sampled: true)
      event = subject.event_from_transaction(transaction)
  ...
  describe "#event_from_exception" do
    ...
    context 'for a nested exception type' do
      ...
      let(:exception) { Haystack::Test::Exception.new(message) }

      it 'sends the module name as part of the exception info' do
        expect(hash[:exception][:values][0][:module]).to eq('Haystack::Test')
      end
    end

    describe "exception types test" do
      context 'for a Haystack::Error' do
        let(:exception) { Haystack::Error.new }
        it 'does not create an event' do
          expect(subject.event_from_exception(exception)).to be_nil
        end
      end

      context 'for a Haystack::ExternalError' do
        let(:exception) { Haystack::ExternalError.new }
        it 'does not create an event' do
          expect(subject.event_from_exception(exception)).to be_nil
        end
      end
      ...
      context 'for an excluded exception type' do
        ...
        context "invalid exclusion type" do
          it 'returns Haystack::ErrorEvent' do
            config.excluded_exceptions << nil
            config.excluded_exceptions << 1
            config.excluded_exceptions << {}
            expect(subject.event_from_exception(Haystack::Test::BaseExc.new)).to be_a(Haystack::ErrorEvent)
          end
        end

        context "defined by string type" do
          it 'returns nil for a class match' do
            config.excluded_exceptions << 'Haystack::Test::BaseExc'
            expect(subject.event_from_exception(Haystack::Test::BaseExc.new)).to be_nil
          end
          ...
          it 'returns nil for a sub class match' do
            config.excluded_exceptions << 'Haystack::Test::BaseExc'
            expect(subject.event_from_exception(Haystack::Test::SubExc.new)).to be_nil
          end

          it 'returns nil for a tagged class match' do
            config.excluded_exceptions << 'Haystack::Test::ExcTag'
            expect(
              subject.event_from_exception(Haystack::Test::SubExc.new.tap { |x| x.extend(Haystack::Test::ExcTag) })
            ).to be_nil
          end

          it 'returns Haystack::ErrorEvent for an undefined exception class' do
            config.excluded_exceptions << 'Haystack::Test::NonExistentExc'
            expect(subject.event_from_exception(Haystack::Test::BaseExc.new)).to be_a(Haystack::ErrorEvent)
          end
        end
        ...
        context "defined by class type" do
          ...
          it 'returns nil for a tagged class match' do
            config.excluded_exceptions << Haystack::Test::ExcTag
            expect(subject.event_from_exception(Haystack::Test::SubExc.new.tap { |x| x.extend(Haystack::Test::ExcTag) })).to be_nil
          end
        end
        ...
        context "when exclusions overridden with :ignore_exclusions" do
          it 'returns Haystack::ErrorEvent' do
            config.excluded_exceptions << Haystack::Test::BaseExc
            expect(subject.event_from_exception(Haystack::Test::BaseExc.new, { ignore_exclusions: true })).to be_a(Haystack::ErrorEvent)
          end
        end
      end
      ...
      context 'when the exception responds to haystack_context' do
        let(:hash) do
  ...
  describe "#event_from_check_in" do
    ...
    it 'returns an event' do
      ...
      expect(event).to be_a(Haystack::CheckInEvent)
    ...
    it 'returns an event with correct optional attributes from crontab config' do
      event = subject.event_from_check_in(
        ...
        duration: 30,
        check_in_id: "xxx-yyy",
        monitor_config: Haystack::Cron::MonitorConfig.from_crontab("* * * * *")
      )
    ...
    it 'returns an event with correct optional attributes from interval config' do
      event = subject.event_from_check_in(
        ...
        duration: 30,
        check_in_id: "xxx-yyy",
        monitor_config: Haystack::Cron::MonitorConfig.from_interval(30, :minute)
      )
  ...
  describe "#generate_haystack_trace" do
    let(:string_io) { StringIO.new }
    let(:logger) do
    ...
    let(:span) { Haystack::Span.new(transaction: transaction) }

    it "generates the trace with given span and logs correct message" do
      expect(subject.generate_haystack_trace(span)).to eq(span.to_haystack_trace)
      expect(string_io.string).to match(
        /\[Tracing\] Adding haystack-trace header to outgoing request: #{span.to_haystack_trace}/
      )
    end
    ...
    context "with config.propagate_traces = false" do
      ...
      it "returns nil" do
        expect(subject.generate_haystack_trace(span)).to eq(nil)
      end
    end
  end
  ...
  describe "#generate_baggage" do
    ...
    let(:baggage) do
      Haystack::Baggage.from_incoming_header(
        "other-vendor-value-1=foo;bar;baz, haystack-trace_id=771a43a4192642f0b136d5159a501700, "\
        "haystack-public_key=49d0f7386ad645858ae85020e393bef3, haystack-sample_rate=0.01337, "\
        "haystack-user_id=Am%C3%A9lie, other-vendor-value-2=foo;bar;"
      )
    end

    let(:span) do
      hub = Haystack::Hub.new(subject, Haystack::Scope.new)
      transaction = Haystack::Transaction.new(name: "test transaction",
                                            baggage: baggage,
                                            hub: hub,
                                            sampled: true)

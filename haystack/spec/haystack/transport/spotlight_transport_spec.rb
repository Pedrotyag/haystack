...
RSpec.describe Haystack::SpotlightTransport do
  ...
  it 'logs a debug message during initialization' do
    ...
    expect(string_io.string).to include('haystack: [Spotlight] initialized for url http://localhost:8969/stream')
  end
  ...
  describe '#send_data' do
    it 'fails a maximum of three times and logs disable once' do
      ...
      expect(string_io.string.scan('haystack: [Spotlight] disabling because of too many request failures').size).to eq(1)
    end
  end
end

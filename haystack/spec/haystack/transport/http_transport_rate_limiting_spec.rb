...
RSpec.describe "rate limiting" do
  ...
  let(:configuration) { Haystack.configuration }
  let(:client) { Haystack.get_current_client }
  let(:data) do
    subject.envelope_from_event(client.event_from_message("foobarbaz").to_hash).to_s
  end

  subject { Haystack::HTTPTransport.new(configuration) }
  ...
  describe "rate limit header processing" do
    ...
    shared_examples "rate limiting headers handling" do
      context "with x-haystack-rate-limits header" do
        now = Time.now
        ...
        [
          ...
          context "with header value: '#{pair[:header]}'" do
            let(:headers) do
              { "x-haystack-rate-limits" => pair[:header] }
            end
        ...
        context "when receiving a greater value for a present category" do
          let(:headers) do
            { "x-haystack-rate-limits" => "120:error:organization" }
          end
        ...
        context "when receiving a smaller value for a present category" do
          let(:headers) do
            { "x-haystack-rate-limits" => "10:error:organization" }
          end
      ...
      context "with both x-haystack-rate-limits and retry-after headers" do
        let(:headers) do
          { "x-haystack-rate-limits" => "42:error:organization", "retry-after" => "42" }
        end

        it "parses x-haystack-rate-limits first" do
          now = Time.now

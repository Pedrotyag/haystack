...
RSpec.describe Haystack::Configuration do
  describe "#capture_exception_frame_locals" do
    ...
    it "prints deprecation message when being assigned" do
      ...
      expect(string_io.string).to include(
        "WARN -- haystack: `capture_exception_frame_locals` is now deprecated in favor of `include_local_variables`."
      )
    end
  end
  ...
  describe "#background_worker_threads" do
    it "sets to have of the processors count" do
      allow_any_instance_of(Haystack::Configuration).to receive(:processor_count).and_return(8)
      expect(subject.background_worker_threads).to eq(4)
    end

    it "sets to 1 with only 1 processor" do
      allow_any_instance_of(Haystack::Configuration).to receive(:processor_count).and_return(1)
  ...
  describe "#csp_report_uri" do
    ...
    context "when the DSN is present" do
      before do
        ...
        subject.dsn = Haystack::TestHelper::DUMMY_DSN
      end

      it "returns the uri" do
        expect(subject.csp_report_uri).to eq("http://haystack.localdomain/api/42/security/?haystack_key=12345")
      end

      it "adds haystack_release param when there's release information" do
        subject.release = "test-release"
        expect(subject.csp_report_uri).to eq("http://haystack.localdomain/api/42/security/?haystack_key=12345&haystack_release=test-release")
      end

      it "adds haystack_environment param when there's environment information" do
        subject.environment = "test-environment"
        expect(subject.csp_report_uri).to eq("http://haystack.localdomain/api/42/security/?haystack_key=12345&haystack_environment=test-environment")
      end
    end
  end
  ...
  describe "#cron" do
    it "returns an initialized Cron::Configuration object" do
      expect(subject.cron).to be_a(Haystack::Cron::Configuration)
  ...
  describe "#spotlight" do
    ...
    it 'uses `HAYSTACK_SPOTLIGHT` env variable for truthy' do
      ENV['HAYSTACK_SPOTLIGHT'] = 'on'

      expect(subject.spotlight).to eq(true)
    end

    it 'uses `HAYSTACK_SPOTLIGHT` env variable for falsy' do
      ENV['HAYSTACK_SPOTLIGHT'] = '0'

      expect(subject.spotlight).to eq(false)
    end

    it 'uses `HAYSTACK_SPOTLIGHT` env variable for custom value' do
      ENV['HAYSTACK_SPOTLIGHT'] = 'https://my.remote.server:8080/stream'
  ...
  describe "#debug" do
    ...
    it 'uses `HAYSTACK_DEBUG` env variable for truthy' do
      ENV['HAYSTACK_DEBUG'] = 'on'

      expect(subject.debug).to eq(true)
    end

    it 'uses `HAYSTACK_DEBUG` env variable for falsy' do
      ENV['HAYSTACK_DEBUG'] = '0'

      expect(subject.debug).to eq(false)
    end

    it 'uses `HAYSTACK_DEBUG` env variable to turn on random value' do
      ENV['HAYSTACK_DEBUG'] = 'yabadabadoo'
  ...
  context 'being initialized with a current environment' do
    before(:each) do
      ...
      subject.dsn = 'http://12345:67890@haystack.localdomain:3000/haystack/42'
    end
  ...
  context 'being initialized without a current environment' do
    ...
    it 'uses `HAYSTACK_CURRENT_ENV` env variable' do
      ENV['HAYSTACK_CURRENT_ENV'] = 'set-with-haystack-current-env'
      ENV['HAYSTACK_ENVIRONMENT'] = 'set-with-haystack-environment'
      ENV['RAILS_ENV'] = 'set-with-rails-env'
      ENV['RACK_ENV'] = 'set-with-rack-env'

      expect(subject.environment).to eq('set-with-haystack-current-env')
    end

    it 'uses `HAYSTACK_ENVIRONMENT` env variable' do
      ENV['HAYSTACK_ENVIRONMENT'] = 'set-with-haystack-environment'
      ENV['RAILS_ENV'] = 'set-with-rails-env'
      ENV['RACK_ENV'] = 'set-with-rack-env'

      expect(subject.environment).to eq('set-with-haystack-environment')
    end

    it 'uses `RAILS_ENV` env variable' do
      ENV['HAYSTACK_CURRENT_ENV'] = nil
      ENV['RAILS_ENV'] = 'set-with-rails-env'
      ENV['RACK_ENV'] = 'set-with-rack-env'
  ...
  context "with the new Haystack 9 DSN format" do
    # Basically the same as before, without a secret
    before(:each) do
      subject.dsn = "https://66260460f09b5940498e24bb7ce093a0@haystack.io/42"
    end
  ...
  describe '#exception_class_allowed?' do
    ...
    context 'with custom excluded_exceptions' do
      ...
      context 'when the raised exception is a Haystack::Error' do
        let(:incoming_exception) { Haystack::Error.new }
        it 'returns false' do
  ...
  describe '.add_post_initialization_callback' do
    class HaystackConfigurationSample < Haystack::Configuration
    ...
    subject(:configuration) { HaystackConfigurationSample }
  ...
  describe "#instrumenter" do
    it "returns :haystack by default" do
      expect(subject.instrumenter).to eq(:haystack)
    end

    it "can be set to :haystack" do
      subject.instrumenter = :haystack
      expect(subject.instrumenter).to eq(:haystack)
    end
    ...
    it "defaults to :haystack if invalid" do
  ...
  describe "#profiler_class=" do
    it "sets the profiler class to Vernier when it's available", when: :vernier_installed? do
      subject.profiler_class = Haystack::Vernier::Profiler
      expect(subject.profiler_class).to eq(Haystack::Vernier::Profiler)
    end

    it "sets the profiler class to StackProf when Vernier is not available", when: { ruby_version?: [:<, "3.2"] } do
      expect { subject.profiler_class = Haystack::Vernier::Profiler }
        .to raise_error(
          ArgumentError,

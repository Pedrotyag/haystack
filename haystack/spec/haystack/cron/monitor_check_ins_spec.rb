...
RSpec.describe Haystack::Cron::MonitorCheckIns do
  ...
  context 'including mixin' do
    context 'without patching' do
      ...
      it 'does not call capture_check_in' do
        expect(Haystack).not_to receive(:capture_check_in)
        job.perform(1)
      end
    ...
    context 'patched with default options' do
      ...
      it 'records 2 check-in events' do
        ...
        in_progress_event = haystack_events.first
    ...
    context 'with very long class name' do
      ...
      it 'truncates from the beginning and parameterizes slug' do
        slug = VeryLongOuterModule::VeryVeryVeryVeryLongInnerModule::Job.haystack_monitor_slug
        expect(slug).to eq('ongoutermodule-veryveryveryverylonginnermodule-job')
      end
    end

    context 'patched with monitor config' do
      let(:monitor_config) { Haystack::Cron::MonitorConfig.from_interval(1, :minute) }
      ...
      before do
        ...
        job_class = Class.new do
          ...
          haystack_monitor_check_ins slug: 'custom_slug', monitor_config: config
      ...
      it 'has correct custom options' do
        expect(Job.haystack_monitor_slug).to eq('custom_slug')
        expect(Job.haystack_monitor_config).to eq(monitor_config)
      end
    ...
    context 'patched with custom options with exception' do
      let(:monitor_config) { Haystack::Cron::MonitorConfig.from_crontab('5 * * * *') }
      ...
      it 'calls capture_check_in twice with error status and re-raises exception' do
        ...
        error_event = haystack_events.last

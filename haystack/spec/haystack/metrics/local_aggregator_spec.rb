...
RSpec.describe Haystack::Metrics::LocalAggregator do
  ...
  describe '#add' do
    it 'creates new GaugeMetric and adds it to bucket if key not existing' do
      expect(Haystack::Metrics::GaugeMetric).to receive(:new).with(10).and_call_original
    ...
    it 'adds value to existing GaugeMetric' do
      ...
      expect(Haystack::Metrics::GaugeMetric).not_to receive(:new)

...
RSpec.describe Haystack::Metrics::Aggregator do
  ...
  subject { Haystack.metrics_aggregator }
  ...
  describe '#add' do
    ...
    context 'with running transaction' do
      it 'has the transaction name in tags serialized in the bucket metric key' do
        Haystack.get_current_scope.set_transaction_name('foo')
        subject.add(:c, 'incr', 1)
        _, _, _, tags = subject.buckets.values.first.keys.first
      ...
      it 'does not has the low quality transaction name in tags serialized in the bucket metric key' do
        Haystack.get_current_scope.set_transaction_name('foo', source: :url)
        subject.add(:c, 'incr', 1)
        _, _, _, tags = subject.buckets.values.first.keys.first
    ...
    it 'creates a new CounterMetric instance if not existing' do
      expect(Haystack::Metrics::CounterMetric).to receive(:new).and_call_original
      subject.add(:c, 'incr', 1)

      metric = subject.buckets.values.first.values.first
      expect(metric).to be_a(Haystack::Metrics::CounterMetric)
    ...
    it 'reuses the existing CounterMetric instance' do
      ...
      expect(Haystack::Metrics::CounterMetric).not_to receive(:new)
    ...
    it 'creates a new DistributionMetric instance if not existing' do
      expect(Haystack::Metrics::DistributionMetric).to receive(:new).and_call_original
      subject.add(:d, 'dist', 1)

      metric = subject.buckets.values.first.values.first
      expect(metric).to be_a(Haystack::Metrics::DistributionMetric)
      expect(metric.value).to eq([1.0])
    end

    it 'creates a new GaugeMetric instance if not existing' do
      expect(Haystack::Metrics::GaugeMetric).to receive(:new).and_call_original
      subject.add(:g, 'gauge', 1)

      metric = subject.buckets.values.first.values.first
      expect(metric).to be_a(Haystack::Metrics::GaugeMetric)
      expect(metric.serialize).to eq([1.0, 1.0, 1.0, 1.0, 1])
    end

    it 'creates a new SetMetric instance if not existing' do
      expect(Haystack::Metrics::SetMetric).to receive(:new).and_call_original
      subject.add(:s, 'set', 1)

      metric = subject.buckets.values.first.values.first
      expect(metric).to be_a(Haystack::Metrics::SetMetric)
    ...
    describe 'local aggregation for span metric summaries' do
      it 'does nothing without an active scope span' do
        expect_any_instance_of(Haystack::Metrics::LocalAggregator).not_to receive(:add)
        subject.add(:c, 'incr', 1)
      end

      context 'with running transaction and active span' do
        let(:span) { Haystack.start_transaction }

        before do
          Haystack.get_current_scope.set_span(span)
          Haystack.get_current_scope.set_transaction_name('metric', source: :view)
        end

        it 'does nothing if transaction name is low quality' do
          expect_any_instance_of(Haystack::Metrics::LocalAggregator).not_to receive(:add)

          Haystack.get_current_scope.set_transaction_name('/123', source: :url)
          subject.add(:c, 'incr', 1)
        end
  ...
  describe '#flush' do
    ...
    context 'with pending buckets' do
      ...
      context 'with force' do
        ...
        it 'captures the envelope' do
          expect(Haystack.background_worker).to receive(:perform)
          subject.flush(force: true)
        end
    ...
    context 'sanitization' do
      it 'sanitizes the metric key' do
        ...
        statsd, metrics_meta = haystack_envelopes.first.items.map(&:payload)
      ...
      it 'sanitizes tag keys and values' do
        ...
        statsd = haystack_envelopes.first.items.first.payload

...
RSpec.describe Haystack::Event do
  ...
  describe "#initialize" do
    ...
    it "initializes a Event with correct default values" do
      ...
      expect(event.sdk).to eq("name" => "haystack.ruby", "version" => Haystack::VERSION)
  ...
  describe "#inspect" do
    let(:client) do
      Haystack::Client.new(configuration)
    end
  ...
  context 'rack context specified', when: :rack_available? do
    ...
    before do
      ...
      Haystack.get_current_scope.set_rack_env(
        'REQUEST_METHOD' => 'POST',
    ...
    let(:event) do
      Haystack::Event.new(configuration: Haystack.configuration)
    end
    ...
    context "with config.send_default_pii = true" do
      ...
      it "adds correct data" do
        Haystack.get_current_scope.apply_to_event(event)
      ...
      it "doesn't overwrite already set ip address" do
        Haystack.set_user({ ip_address: "3.3.3.3" })
        Haystack.get_current_scope.apply_to_event(event)
      ...
      context "with config.trusted_proxies = [\"2.2.2.2\"]" do
        before do
          Haystack.configuration.trusted_proxies = ["2.2.2.2"]
        end
  ...
  describe '#to_json_compatible' do
    subject do
      Haystack::Event.new(configuration: configuration).tap do |event|
        event.extra = {

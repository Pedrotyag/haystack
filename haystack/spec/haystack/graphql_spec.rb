...
RSpec.describe 'GraphQL' do
  it 'adds the graphql patch to registered patches' do
    expect(Haystack.registered_patches.keys).to include(:graphql)
  end
  ...
  context 'when patch enabled' do
    if with_graphql
      describe 'with graphql gem' do
        ...
        it 'enables the haystack tracer' do
          expect(MySchema.trace_modules_for(:default)).to include(::GraphQL::Tracing::HaystackTrace)
        end
        ...
        it 'adds graphql spans to the transaction' do
          ...
          expect(last_haystack_event.transaction).to eq('GraphQL/query.foobar')

          execute_span = last_haystack_event.spans.find { |s| s[:op] == 'graphql.execute' }

...
RSpec.describe 'Haystack::Rack::CaptureExceptions', when: :rack_available? do
  ...
  describe "exceptions capturing" do
    ...
    it 'captures the exception from direct raise' do
      ...
      expect(env["haystack.error_event_id"]).to eq(event[:event_id])
    ...
    it 'sets the transaction and rack env' do
      ...
      expect(event.to_hash.dig(:request, :url)).to eq("http://example.org/test")
      expect(Haystack.get_current_scope.transaction_name).to be_nil
    ...
    it 'passes rack/lint' do
      ...
      stack = Haystack::Rack::CaptureExceptions.new(Rack::Lint.new(app))
      expect { stack.call(env) }.to_not raise_error
      expect(env.key?("haystack.error_event_id")).to eq(false)
    end
    ...
    describe "state encapsulation" do
      before do
        Haystack.configure_scope { |s| s.set_tags(tag_1: "don't change me") }
        Haystack.configuration.breadcrumbs_logger = [:haystack_logger]
      end
      ...
      it "only contains the breadcrumbs of the request" do
        ...
        request_1 = lambda do |e|
          ...
          Haystack.capture_message("test")
          [200, {}, ["ok"]]
        end

        app_1 = Haystack::Rack::CaptureExceptions.new(request_1)
      ...
      it "doesn't pollute the top-level scope" do
        request_1 = lambda do |e|
          Haystack.configure_scope { |s| s.set_tags({ tag_1: "foo" }) }
          Haystack.capture_message("test")
          [200, {}, ["ok"]]
        end
        app_1 = Haystack::Rack::CaptureExceptions.new(request_1)
        ...
        expect(Haystack.get_current_scope.tags).to eq(tag_1: "don't change me")
      end
      it "doesn't pollute other request's scope" do
        ...
        request_2 = proc do |e|
          Haystack.configure_scope { |s| s.set_tags({ tag_2: "bar" }) }
          e['rack.exception'] = Exception.new
          [200, {}, ["ok"]]
        end
        app_2 = Haystack::Rack::CaptureExceptions.new(request_2)
        app_2.call(env)
  ...
  describe "performance monitoring" do
    ...
    context "when haystack-trace header is sent" do
      ...
      let(:stack) do
        Haystack::Rack::CaptureExceptions.new(
          ->(_) do
            [200, {}, ["ok"]]
          end
        )
      ...
      context "when the baggage header is sent" do
        ...
        it "has the dynamic_sampling_context on the TransactionEvent" do
          expect(Haystack::Transaction).to receive(:new).
    ...
    context "when the transaction is sampled" do
      ...
      it "starts a transaction and finishes it" do
        ...
        transaction = last_haystack_event
        expect(transaction.type).to eq("transaction")
        expect(transaction.transaction).to eq("/test")
        expect(transaction.transaction_info).to eq({ source: :url })
        expect(transaction.timestamp).not_to be_nil
      ...
      describe "Haystack.with_child_span" do
        it "sets nested spans correctly under the request's transaction" do
          app = ->(_) do
            Haystack.with_child_span(op: "first level") do
              Haystack.with_child_span(op: "second level") do
                [200, {}, ["ok"]]
              end
            end
          end
    ...
    context "when traces_sample_rate is not set" do
      before do
        Haystack.configuration.traces_sample_rate = nil
      end
      ...
      context "when haystack-trace header is sent" do
        ...
        it "doesn't cause the transaction to be recorded" do
          env["HTTP_HAYSTACK_TRACE"] = external_transaction.to_haystack_trace
  ...
  describe "tracing without performance" do
    ...
    let(:stack) do
      app = ->(_e) { raise exception }
      Haystack::Rack::CaptureExceptions.new(app)
    end
    ...
    it "captures exception with correct DSC and trace context" do
      ...
      trace_context = last_haystack_event.contexts[:trace]
      ...
      expect(last_haystack_event.dynamic_sampling_context).to eq(incoming_prop_context.get_dynamic_sampling_context)
    end
  end
  ...
  describe "session capturing" do
    context "when auto_session_tracking is false" do
      ...
      it "passthrough" do
        ...
        expect_any_instance_of(Haystack::Hub).not_to receive(:start_session)
        expect(Haystack.session_flusher).to be_nil
        stack = Haystack::Rack::CaptureExceptions.new(app)
        stack.call(env)
    ...
    context "tracks sessions by default" do
      ...
      it "collects session stats and sends envelope with aggregated sessions" do
        ...
        expect(Haystack.session_flusher).not_to be_nil
        ...
        Timecop.freeze(now) do
          ...
          Haystack.session_flusher.flush

          expect(haystack_envelopes.count).to eq(1)
          envelope = haystack_envelopes.first
  ...
  shared_examples "a profiled transaction" do
    it "collects a profile", retry: 3 do
      ...
      thread_id_mapping = {
        Haystack::Profiler => "0",
        Haystack::Vernier::Profiler => Thread.current.object_id.to_s
      }

      expect(profile[:transaction][:active_thread_id]).to eq(thread_id_mapping[Haystack.configuration.profiler_class])
    ...
    context "when profiling is enabled" do
      ...
      before do
        perform_basic_setup do |config|
          ...
          config.profiles_sample_rate = 1.0
          config.release = "test-release"
          config.profiler_class = Haystack::Vernier::Profiler

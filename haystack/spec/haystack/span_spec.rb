...
RSpec.describe Haystack::Span do
  let(:hub) do
    client = Haystack::Client.new(Haystack::Configuration.new)
  ...
  describe "#to_haystack_trace" do
    it "returns correctly-formatted value" do
      haystack_trace = subject.to_haystack_trace

      expect(haystack_trace).to eq("#{subject.trace_id}-#{subject.span_id}-1")
      expect(haystack_trace).to match(Haystack::PropagationContext::HAYSTACK_TRACE_REGEXP)
    end
    ...
    context "without sampled value" do
      ...
      it "doesn't contain the sampled flag" do
        ...
        expect(haystack_trace).to eq("#{subject.trace_id}-#{subject.span_id}-")
        expect(haystack_trace).to match(Haystack::PropagationContext::HAYSTACK_TRACE_REGEXP)
      end
    end
  end
  ...
  describe "#to_baggage" do
    ...
    subject do
      ...
      Haystack::Transaction.new(hub: Haystack.get_current_hub, baggage: baggage).start_child
    end

    it "propagates haystack baggage values" do
  ...
  describe "#get_dynamic_sampling_context" do
    ...
    it "propagates haystack dynamic_sampling_context" do
  ...
  describe "#start_child" do
    ...
    context "when the parent span has a span_recorder" do
      subject do
        ...
        Haystack::Transaction.new(hub: Haystack.get_current_hub).start_child
      end

...
class HaystackGenerator < ::Rails::Generators::Base
  class_option :dsn, type: :string, desc: "Haystack DSN"

  class_option :inject_meta, type: :boolean, default: true, desc: "Inject meta tag into layout"

  def copy_initializer_file
    dsn = options[:dsn] ? "'#{options[:dsn]}'" : "ENV['HAYSTACK_DSN']"

    create_file "config/initializers/haystack.rb", <<~RUBY
      # frozen_string_literal: true
  ...
  def inject_code_into_layout
    ...
    inject_into_file "app/views/layouts/application.html.erb", before: "</head>\n" do
      "  <%= Haystack.get_trace_propagation_meta.html_safe %>\n  "
    end
  end
end

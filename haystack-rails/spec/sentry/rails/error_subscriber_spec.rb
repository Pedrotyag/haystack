...
RSpec.describe Haystack::Rails::ErrorSubscriber, skip: Rails.version.to_f < 7.0 ? "ActiveSupport::ErrorReporter not available until Rails 7" : false do
  describe "#report" do
    ...
    it 'sets handled tag' do
      expect(Haystack::Rails).to receive(:capture_exception) do |_, tags:, **_|
    ...
    context 'when source is not nil' do
      it 'does not send event when source is skipped' do
        expect(Haystack::Rails).not_to receive(:capture_exception)
      ...
      it 'sends event when source is not skipped' do
        expect(Haystack::Rails).to receive(:capture_exception)
    ...
    context 'when passed a context with tags key' do
      context 'when tags is a Hash' do
        ...
        it 'does not pass the tags to the context' do
          expect(Haystack::Rails).to receive(:capture_exception) do |_, contexts:, **_|
    ...
    context 'when passed a context with hint key' do
      context 'when hint is a Hash' do
        it 'merges the hint into the event' do
          expect(Haystack::Rails).to receive(:capture_exception) do |_, hint:, **_|

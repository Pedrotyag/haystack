...
RSpec.describe Haystack::Sidekiq::Cron::Job do
  let(:processor) do
    new_processor
  end
  ...
  it 'patches HappyWorkerForCron' do
    expect(HappyWorkerForCron.ancestors).to include(Haystack::Cron::MonitorCheckIns)
    expect(HappyWorkerForCron.haystack_monitor_slug).to eq('happy')
    expect(HappyWorkerForCron.haystack_monitor_config).to be_a(Haystack::Cron::MonitorConfig)
    expect(HappyWorkerForCron.haystack_monitor_config.schedule).to be_a(Haystack::Cron::MonitorSchedule::Crontab)
    expect(HappyWorkerForCron.haystack_monitor_config.schedule.value).to eq('* * * * *')
  end

  it 'patches HappyWorkerWithHumanReadableCron' do
    expect(HappyWorkerWithHumanReadableCron.ancestors).to include(Haystack::Cron::MonitorCheckIns)
    expect(HappyWorkerWithHumanReadableCron.haystack_monitor_slug).to eq('human_readable_cron')
    expect(HappyWorkerWithHumanReadableCron.haystack_monitor_config).to be_a(Haystack::Cron::MonitorConfig)
    expect(HappyWorkerWithHumanReadableCron.haystack_monitor_config.schedule).to be_a(Haystack::Cron::MonitorSchedule::Crontab)
    expect(HappyWorkerWithHumanReadableCron.haystack_monitor_config.schedule.value).to eq('*/5 * * * *')
  end

  it 'patches HappyWorkerWithSymbolName' do
    expect(HappyWorkerWithSymbolName.ancestors).to include(Haystack::Cron::MonitorCheckIns)
    expect(HappyWorkerWithSymbolName.haystack_monitor_slug).to eq('symbol_name')
    expect(HappyWorkerWithSymbolName.haystack_monitor_config).to be_a(Haystack::Cron::MonitorConfig)
    expect(HappyWorkerWithSymbolName.haystack_monitor_config.schedule).to be_a(Haystack::Cron::MonitorSchedule::Crontab)
    expect(HappyWorkerWithSymbolName.haystack_monitor_config.schedule.value).to eq('* * * * *')
  end
  ...
  it 'does not patch ReportingWorker because of invalid schedule' do
    expect(ReportingWorker.ancestors).not_to include(Haystack::Cron::MonitorSchedule)
  end
  ...
  describe 'sidekiq-cron' do
    it 'adds job to sidekiq within transaction' do
      ...
      job.send(Haystack::Sidekiq::Cron::Job.enqueueing_method)

...
RSpec.describe Haystack::Sidekiq::HaystackContextServerMiddleware do
  ...
  context "with tracing enabled" do
    ...
    context "with trace_propagation_headers" do
      let(:parent_transaction) { Haystack.start_transaction(op: "sidekiq") }

      it "starts the transaction from it" do
        trace_propagation_headers = { "haystack-trace" => parent_transaction.to_haystack_trace }
...
RSpec.describe Haystack::Sidekiq::HaystackContextClientMiddleware do
  include_context "sidekiq"
  ...
  it "does not add user to the job if they're absent in the current scope" do
    ...
    expect(queue.first["haystack_user"]).to be_nil
  end
  ...
  describe "with user" do
    ...
    it "sets user of the current scope to the job" do
      ...
      expect(queue.size).to be(1)
      expect(queue.first["haystack_user"]).to eq(user)
    end
  end

  describe "with transaction" do
    let(:transaction) { Haystack.start_transaction(op: "sidekiq") }
    ...
    it "sets the correct trace_propagation_headers linked to the transaction" do
      ...
      expect(headers["haystack-trace"]).to eq(transaction.to_haystack_trace)

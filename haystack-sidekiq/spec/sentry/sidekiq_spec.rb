...
RSpec.describe Haystack::Sidekiq do
  ...
  it "registers error handlers and middlewares" do
    if WITH_SIDEKIQ_7
      ...
      expect(config.server_middleware.entries.map(&:klass)).to include(described_class::HaystackContextServerMiddleware)
      expect(config.client_middleware.entries.map(&:klass)).to include(described_class::HaystackContextClientMiddleware)
    else
      expect(Sidekiq.error_handlers).to include(described_class::ErrorHandler)
      expect(Sidekiq.server_middleware.entries.first.klass).to eq(described_class::HaystackContextServerMiddleware)
      expect(Sidekiq.client_middleware.entries.first.klass).to eq(described_class::HaystackContextClientMiddleware)
    end
  end
  ...
  it "captures exception raised in the worker" do
    ...
    expect(event[:sdk]).to eq({ name: "haystack.ruby.sidekiq", version: described_class::VERSION })
  ...
  describe "context cleanup" do
    it "cleans up context from processed jobs" do
      ...
      expect(Haystack.get_current_scope.tags).to be_empty
    end
  ...
  context "with config.report_after_job_retries = true" do
    before do
      Haystack.configuration.sidekiq.report_after_job_retries = true
    end

    ...
    module Scheduler
      def new_job(name, interval_type, config, schedule, options)
        ...
        monitor_config =
          ...
          when "cron"
            ...
            if timezone
              ...
              Haystack::Cron::MonitorConfig.from_crontab(cron_without_timezone, timezone: timezone.name)
            else
              Haystack::Cron::MonitorConfig.from_crontab(schedule)
            end
          when "every", "interval"
            Haystack::Cron::MonitorConfig.from_interval(rufus_job.frequency.to_i / 60, :minute)
          end
        ...
        unless klass_const.send(:ancestors).include?(Haystack::Cron::MonitorCheckIns)
          ...
          slug = klass_const.send(:haystack_monitor_slug, name: name)
          klass_const.send(:haystack_monitor_check_ins,
                           slug: slug,
                           monitor_config: monitor_config)

          ::Sidekiq.logger.info "Injected Haystack Crons monitor checkins into #{klass}"
        end
...
Haystack.register_patch(:sidekiq_scheduler, Haystack::SidekiqScheduler::Scheduler, ::SidekiqScheduler::Scheduler)

  ...
  module Sidekiq
    class ErrorHandler
      ...
      def call(ex, context, sidekiq_config = nil)
        ...
        context_filter = Haystack::Sidekiq::ContextFilter.new(context)
        ...
        # If Haystack is configured to only report an error _after_ all retries have been exhausted,
        # and if the job is retryable, and have not exceeded the retry_limit,
        # return early.
        if Haystack.configuration.sidekiq.report_after_job_retries && retryable?(context)
        ...
        Haystack::Sidekiq.capture_exception(
          ex,

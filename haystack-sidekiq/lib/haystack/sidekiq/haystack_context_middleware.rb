...
require "haystack/sidekiq/context_filter"
...
module Haystack
  module Sidekiq
    ...
    class HaystackContextServerMiddleware
      include Haystack::Sidekiq::Helpers
      ...
      def call(worker, job, queue)
        ...
        context_filter = Haystack::Sidekiq::ContextFilter.new(job)

        Haystack.clone_hub_to_current_thread
        scope = Haystack.get_current_scope
        if (user = job["haystack_user"])
          scope.set_user(user)
        end
    ...
    class HaystackContextClientMiddleware
      include Haystack::Sidekiq::Helpers

      def call(worker_class, job, queue, _redis_pool)
        return yield unless Haystack.initialized?

        user = Haystack.get_current_scope.user
        job["haystack_user"] = user unless user.empty?
        job["trace_propagation_headers"] ||= Haystack.get_trace_propagation_headers

        Haystack.with_child_span(op: "queue.publish", description: worker_class.to_s) do |span|
